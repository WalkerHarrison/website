...
The `gtfsway` package is a life-saver here since it converts the [Google transit feed](https://developers.google.com/transit/gtfs-realtime) into comprehensible R objects, although I did rewrite the `gtfs_tripUpdates` function to drill down on specific routes in the interest of speed:
...
The logic for the second leg is similar, although now we have to record the when exactly the first leg is finishing up each day, since these `transfers` will determine the window during which our second leg can actually start.
