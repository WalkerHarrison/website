...
You'll notice a few prominent features of these histograms. Most of them have single peaks somewhere between 2 and 4 minutes -- these are the most likely wait times, but the average is dragged up in each instance by the characteristic right skew of a non-negative variable. It looks like the MTA can achieve much more consistent wait times at points of origin like the express 7 at Hudson Yards or the shuttle at Times Square. 
...
<ul>
  ...
  <li>once passengers board the train, that data point is recorded on a scatterplot</li>
</ul>

Below such a simulation is visualized on the left along with a dynamic, updating scatterplot on the right. For those curious, it's written in d3 -- mostly using prompts I gave to ChatGPT-- and [hosted on Observable](https://observablehq.com/@wamanjr/good){target="_blank"}.
...
If you let the simulation run for long enough (and using the speed multiplier certainly helps), a sufficient number of outlier delays occur to fill in some points on the far right of the graph. With such sparse data in that region, the red smoothed trendline is forced to bend up toward the longer wait times. In other words, there are certain degrees of crowdedness in this simulation that can only imply extreme wait times.
