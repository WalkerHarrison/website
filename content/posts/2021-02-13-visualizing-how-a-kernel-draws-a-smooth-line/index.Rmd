...
So how are we gonna trace a line through this data? Well for any point on this theoretical curve, the y-value will be a weighted mean of nearby points. The way we define those weights is with a [Gaussian kernel](https://en.wikipedia.org/wiki/Kernel_smoother#Gaussian_kernel_smoother){target="_blank"}, which uses the useful "bell" shape of a normal distribution to assign greater influence to the existing points that are close to our incoming one.
