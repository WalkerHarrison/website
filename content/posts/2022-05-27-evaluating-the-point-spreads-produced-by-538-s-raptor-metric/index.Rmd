...
All of that is to say that I don't expect RAPTOR to be good at predicting NBA games, especially if they don't have up-to-the-minute injury news. But if you're going to throw up point spreads, I can't help but see how accurate they are and compare them to the famously sharp lines put out by sportsbooks. 
...
page <- read_html("/Users/walkerharrison/Desktop/raptor.html")
...
It would appear that RAPTOR spreads and win probabilities could use a bump toward the away team. Let's now compare them to the famously sharp lines offered by sportsbooks, which I'm sourcing from [sportsbooksreviewsonline.com](https://www.sportsbookreviewsonline.com/scoresoddsarchives/nba/nbaoddsarchives.htm){target="_blank"}. While spreads will vary by as much as a point between different sportsbooks, they're typically close enough that consensus lines like these will be sufficient for the analysis. Two of the 1,230 regular season games were missing lines, but I used Action Network spreads to fill those in.
...
sportsbook_raw <- readxl::read_xlsx("/Users/walkerharrison/Downloads/nba odds 2021-22.xlsx")
...
A naive approach would be to simply bet whenever we felt like we had an advantage, according to the sportsbook lines. So if FiveThirtyEight was offering a spread of -3.5/+3.5 for that Bucks vs. Cavs game and the Vegas line was -5.5/+5.5, we would bet on the Bucks -3.5 since that spread undervalued the Hawks according to our information. The same applies to the moneylines.
...
rbind(kelly_spreads, kelly_ML) %>% 
  mutate_at(c('wagered', 'profit'), ~formattable::currency(.x, digits = 0L)) %>%
  mutate(ROI = formattable::percent(ROI)) %>%
  kable(caption = "Kelly Bet with $1000 Bankroll")
```
...
I do find it curious that they haven't calibrated their models to eliminate the home team bias, since that seems like a simple enough fix. But as long as they don't open up a sportsbook, I guess it doesn't really matter that much...
