...
[_**Don't like/know/care about R? Click here to skip to the punchline.**_](#skip)
...
make_ladder <- function(word, typo) {
  ...
  # otherwise keep mutating
    ...
    # sample until a closer word is found
...
What does all this get us to? Well we can make a series of ladders (ten, for now) from our original `word` to the `typo`, with each element representing a single character update and a step away from my intentional query and toward gibberish I actually entered. One such ladder is printed below:
...
It appears, however, that turning the 'logy' at the end of the word into 'mogy' was the death knell for many of these ladders, as that's where the cells turn red, indicating a lack of suggested fixes from Google.
...
We implement the same general process that was applied to the ladders, extracting the terms from the tree and using the API to see whether Google will suggest the intended queryios.
...
#search_results <- map(terms, google)
load("/Users/walkerharrison/Desktop/website/content/posts/2020-12-17-pushing-google-s-autocorrect-to-the-limit/search_results_tree.Rdata")
...
g <- tree_df %>%
  ggplot(aes(depth, y)) +
  geom_point() +
  geom_label(aes(label = word, col = corrected)) +
